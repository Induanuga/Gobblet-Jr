...
def init_pieces():
    ...
    return [
        ...
        {"color": BLUE, "pos": [LEFT_X + SPACING_X - 12, ROW1_Y], "radius": MEDIUM_RADIUS, "stack": []},
        {"color": BLUE, "pos": [LEFT_X + SPACING_X * 2, ROW1_Y], "radius": LARGE_RADIUS, "stack": []},
        {"color": BLUE, "pos": [LEFT_X, ROW2_Y], "radius": SMALL_RADIUS, "stack": []},
        {"color": BLUE, "pos": [LEFT_X + SPACING_X - 12, ROW2_Y], "radius": MEDIUM_RADIUS, "stack": []},
        {"color": BLUE, "pos": [LEFT_X + SPACING_X * 2, ROW2_Y], "radius": LARGE_RADIUS, "stack": []},
        # Pink pieces (Right side)
        {"color": PINK, "pos": [RIGHT_X, ROW1_Y], "radius": SMALL_RADIUS, "stack": []},
        {"color": PINK, "pos": [RIGHT_X - SPACING_X + 12, ROW1_Y], "radius": MEDIUM_RADIUS, "stack": []},
        {"color": PINK, "pos": [RIGHT_X - SPACING_X * 2, ROW1_Y], "radius": LARGE_RADIUS, "stack": []},
        {"color": PINK, "pos": [RIGHT_X, ROW2_Y], "radius": SMALL_RADIUS, "stack": []},
        {"color": PINK, "pos": [RIGHT_X - SPACING_X + 12, ROW2_Y], "radius": MEDIUM_RADIUS, "stack": []},
        {"color": PINK, "pos": [RIGHT_X - SPACING_X * 2, ROW2_Y], "radius": LARGE_RADIUS, "stack": []}
    ]
...
def get_grid_cell(x, y):
    """Snaps a position to the nearest grid cell and returns the cell center and the stack of pieces at that cell."""
    if GRID_X <= x <= GRID_X + GRID_SIZE and GRID_Y <= y <= GRID_Y + GRID_SIZE:
        col = (x - GRID_X) // CELL_SIZE
        row = (y - GRID_Y) // CELL_SIZE
        grid_pos = (GRID_X + col * CELL_SIZE + CELL_SIZE // 2, GRID_Y + row * CELL_SIZE + CELL_SIZE // 2)
...
while running:
    ...
    if game_over:
        ...
        line2 = winner_font.render(f"Winner: {'Player 1' if winner == BLUE else 'Player 2'}", True, varx)
        ...
        screen.blit(line2, (WINDOW_WIDTH // 2 - line2.get_width() // 2, start_y + line1.get_height() + 30))
        screen.blit(line3, (WINDOW_WIDTH // 2 - line3.get_width() // 2, start_y + line1.get_height() + line2.get_height() + 70))
    else:
        draw_grid()
        write_text()
        draw_pieces()
    ...
    for event in pygame.event.get():
        ...
        if game_over:
            ...
            if selected_piece is None:
                for piece in pieces:
                    ...
                    if (mx - px) ** 2 + (my - py) ** 2 <= piece["radius"] ** 2:
                        ...
                        if stack and stack[-1] != piece:
                            continue
                        if (turn == "BLUE" and piece["color"] == BLUE) or (turn == "PINK" and piece["color"] == PINK):
                ...
                if new_pos:
                    # Enforce stacking rule: only place on an empty cell or on a piece that is smaller.
                    if stack and selected_piece["radius"] <= stack[-1]["radius"]:
                        # Invalid move: revert to original position.
                        selected_piece["pos"] = selected_piece.get("orig_pos", selected_piece["pos"])
                    else:
